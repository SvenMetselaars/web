...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO results (result_username, result_tries, result_time, result_dif)
            VALUES(?, ?, ?, ?)";
...
function getResults()
{
    ...
    ORDER BY 
        --  first hard than medium than easy
        CASE result_dif
            WHEN 'hard' THEN 1
            WHEN 'medium' THEN 2
            WHEN 'easy' THEN 3
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    foreach ($results as $result) {
        ...
        if (($difficulty === null || $difficulty === $result["result_dif"]) && $amount < 5) {
            $amount++;

            $information[] = [
                // Default to "Unknown" if username is null
                "player" => $result["result_username"] ?? "Unknown",  
                "dif" => $result["result_dif"],
